...
function initOpenClose() {
	...
	ResponsiveHelper.addRange({
		'..1023': {
			...
			off: function() {
				jQuery('.open-close-inner').openClose('destroy');
			}
		}
	});
}
...
function initScrollReveal() {
    ...
    //     mobile: false,
    // });
    $('.anim-fade').addClass("hidden-block").viewportChecker({
      classToAdd: 'visible animated fadeInUp', // Class to add to the elements when they are visible
      offset: 100
    });

    $('.anim-fade-left').addClass("hidden-block").viewportChecker({
      classToAdd: 'visible animated fadeInLeft', // Class to add to the elements when they are visible
      offset: 100
    });

    $('.anim-fade-right').addClass("hidden-block").viewportChecker({
...
// Play Buttons for YouTube Videos (post pages)
function initBtnPlayIframe(){
	$('.btn-play-video').on('click touchstart', function(event){
		event.preventDefault();
		$('.video-fluid').find('iframe').attr('src','');
		$('.video-fluid').removeClass('active');
		var videoBlock = $('.video-fluid'),
			dataVideo = $(this).closest(videoBlock).attr('data-video'),
			iframe = $(this).closest(videoBlock).find('iframe');
		iframe.attr('src', dataVideo + '?&autoplay=1&rel=0&fs=1&showinfo=0&autohide=3&modestbranding=1');
		$(this).parent().addClass('active');
	});

	if( $('.bg-video').length > 0 ) {
		var autoplay = setInterval(function(){
	      var video = $('.promo-section video')[0];
	      if(video.play() == false){
	          video.play();
	      } else {
...
function initSlick() {
	...
	jQuery('.festival-gallery').slick({
		nextArrow: "<button class='slick-next'>next</button>",
		prevArrow: "<button class='slick-prev'>prev</button>",
	})
	.on('afterChange', function(event, slick, currentSlide, nextSlide){
		var getData = $(slick.$list[0]).find('.slide').eq(currentSlide + 1).attr('data-src');
		$(slick.$list[0]).closest('.upcoming-events').css('background','url("images/' + getData + '") no-repeat 50% / cover')
	});
...
// popups init
function initPopups() {
	jQuery('.events-gallery .slide').contentPopup({
		mode: 'click'
	});
	jQuery('.calendar-section .popup-container').contentPopup({
		mode: 'click'
	});
}
